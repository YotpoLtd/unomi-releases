...
public class SegmentServiceImpl extends AbstractServiceImpl implements SegmentService, SynchronousBundleListener {
    ...
    protected List<Segment> allSegments;
    ...
    @Override
    public boolean saveSegment(Segment segment, boolean alwaysOverwrite) {
        definitionsService.resolveConditionType(segment.getCondition());
        persistenceService.validateCondition(segment.getCondition(), new Profile());
        validateSegmentDependencies(segment);
        ...
        segment.setStatus(Segment.EvaluationStatus.IN_PROGRESS_USER);
        return persistenceService.save(segment, null, alwaysOverwrite);
    }
    ...
    private List<String> collectAllSegmentDependencies(Condition condition) {
        ...
        new ParserHelper().visitConditions(condition, currentCondition -> {
            if ("profileSegmentCondition".equals(currentCondition.getConditionTypeId())) {
                results.addAll((List<String>) currentCondition.getParameter("segments"));
            }
        });
        return results;
    }
    ...
    private DependentMetadata getDependentMetadata(Set<Segment> impactedSegments, Set<Scoring> impactedScorings) {
        List<Metadata> segments = new LinkedList<>();
        List<Metadata> scorings = new LinkedList<>();
    ...
    public void updateExistingProfilesForSegment(Segment segment, boolean isInitiatedBySystem) {
        try {
            ...
            Segment.EvaluationStatus evalStatus = isInitiatedBySystem
                    ? Segment.EvaluationStatus.IN_PROGRESS_SYSTEM : Segment.EvaluationStatus.IN_PROGRESS_USER;
            updateSegmentEvaluationStatus(segment, evalStatus);
            ...
            updateSegmentEvaluationStatus(segment, Segment.EvaluationStatus.DONE);
            logger.info("{} profiles updated in {}ms", updatedProfileCount, System.currentTimeMillis() - updateProfilesForSegmentStartTime);
        } catch (Exception e) {
            logger.error("failed to update profile with segment={}, condition={}", segment.getItemId(), segment.getCondition().toString(), e);
            updateSegmentEvaluationStatus(segment, Segment.EvaluationStatus.FAILED);
            throw e;
        }
    }

    @Override
    public void updateSegmentEvaluationStatus(Segment segment, Segment.EvaluationStatus status) {
        logger.debug("Setting segment (id={}) evaluation status to {}", status);
        segment.setStatus(status);
        boolean saveSegmentEvalStatus = persistenceService.save(segment, null, false);
...
//                    dateExprSegments.forEach(segment -> updateExistingProfilesForSegment(segment, true));
                    ...
                    dateExprSegments.forEach(segment -> {
                        try {
                            setSegmentDefinition(segment);
                        } catch (Exception e) {
                            logger.error("Error while updating profiles for DateExpr conditions for segmentId={}", segment.getItemId(), e);
                        }
                    });
